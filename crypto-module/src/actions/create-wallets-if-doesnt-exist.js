...
module.exports = (configs, environment) => async (clientId, cryptoTypes) => {
    ...
    console.log(cryptosToCreateWalletFor);
    ...
    console.log(userCryptoWallets)
    // crypto wallet to create
    const cryptoWalletCryptoTypesToCreate = cryptosToCreateWalletFor.filter(crypto => !userCryptoWallets.map(cryptoWallet => cryptoWallet.cryptoId).includes(crypto.id));
    console.log(cryptoWalletCryptoTypesToCreate);
    // loop through the cryptos and create wallets for each user doesn't have
    const wallets = await Promise.all(cryptoWalletCryptoTypesToCreate.map(async (crypto) => {
    ...
    const cryptoWalletsMadeWithCrypto = await cryptoWalletRepository.findCryptoWalletsWithCryptoWhereIn(database, clientId,
        wallets.map(wallet => wallet.id).concat(userCryptoWallets.map(wallet => wallet.id)));

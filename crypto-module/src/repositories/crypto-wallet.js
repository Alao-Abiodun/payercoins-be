...
const modelName = 'CryptoWallet'

const create = async (database, data = { cryptoId, clientId, balance }, transaction = null) => {
    const options = transaction ? { nest: true, raw: true, transaction } : { nest: true, raw: true };
    return await model.create(database, modelName, data, [{ model: database['Crypto'], as: 'crypto' }], options);
}
